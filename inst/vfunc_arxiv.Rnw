...
Above, ``{\tt f + g}'' returns an error because {\tt f} and {\tt g}
are objects of {\tt S4} class {\tt function}.  It is not possible to
define {\tt Arith} group {\tt S4} methods [in this case, addition,
{\tt +}] so such idiom operates as desired, because the {\tt function}
class is {\em sealed} in {\tt S4}: the definition of new methods is
...
new {\tt S4} class {\tt vf} [for ``virtual function''] which inherits
from {\tt function}, but for which new methods can be defined.  This
device furnishes some ways to apply Arithmetic methods for functions.
...
<<looktwo2>>=
j(sin(sin(0.4) + tan(0.4)), cos(sin(0.4) + tan(0.4)) + exp(sin(0.4) + tan(0.4)))+
k(sin(sin(0.4) + tan(0.4)), cos(sin(0.4) + tan(0.4)) + exp(sin(0.4) + tan(0.4)))
@
